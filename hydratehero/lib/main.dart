...
import 'dart:async';
import 'dart:convert'; // Tambahkan untuk JSON parsing
import 'package:hydratehero/api_service.dart'; // Import file API Service
import 'package:shared_preferences/shared_preferences.dart'; // Untuk menyimpan ID pengguna secara lokal
    ...
    return MaterialApp(
      ...
      routes: {
        ...
        '/personalize': (context) {
          ...
          return PersonalizeScreen(userId: args['userId']);
        },
      ...
      body: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 120), // Spasi antara teks dan tombol
            ElevatedButton(
              onPressed: () {
...
class _SignUpScreenState extends State<SignUpScreen> {
  ...
  void _signUp() async {
    ...
    print('Data yang dikirim: $userData');
    ...
    try {
      ...
      if (response.statusCode == 201) {
        ...
        final userId = responseBody['id_pengguna'];

        // Simpan userId ke SharedPreferences
        final prefs = await SharedPreferences.getInstance();
        await prefs.setInt('userId', userId);

        // Navigasi ke layar personalisasi
        Navigator.pushNamed(
          context,
          '/personalize',
          arguments: {"userId": userId}, // Kirim userId ke PersonalizeScreen
        );
      } else {
        final responseBody = jsonDecode(response.body);
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Sign up gagal: ${responseBody['message']}')),
        );
      }
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
  ...
  void _login() async {
    ...
    if (email.isEmpty || password.isEmpty) {
     ...
     try {
      ...
      final response = await apiService.login(email, password); // Gunakan instance untuk mengakses metode

      if (response.statusCode == 200) {
        final responseBody = jsonDecode(response.body);
        final userId = responseBody['id_pengguna']; // Ambil ID pengguna
        print('Login berhasil! ID Pengguna: $userId');
        
        // Navigasi ke halaman berikutnya dengan ID pengguna
        Navigator.pushNamed(context, '/home', arguments: userId);
      } else {
        final responseBody = jsonDecode(response.body);
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Login gagal: ${responseBody['message']}')),
        );
      }
    } catch (e) {
      print('Login error: $e');
...
class PersonalizeScreen extends StatefulWidget {
  ...
  const PersonalizeScreen({super.key, required this.userId});
...
class _PersonalizeScreenState extends State<PersonalizeScreen> {
  ...
  void savePersonalization() async {
    ...
    final personalizationData = {
      "usia": int.parse(ageController.text),
      "jenis_kelamin": genderController.text.trim().toLowerCase(),
      "tinggi_badan": double.parse(heightController.text),
      "berat_badan": double.parse(weightController.text),
    };

    print('Data yang dikirim: $personalizationData');

    try {
      final response = await http.put(
        Uri.parse('http://localhost:3000/pengguna/${widget.userId}/personalize'),
        headers: {'Content-Type': 'application/json'},
        body: jsonEncode(personalizationData),
      );
      ...
      if (response.statusCode == 200) {
        ...
        final responseBody = jsonDecode(response.body);
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Gagal menyimpan: ${responseBody['message']}')),
        );
      }
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
...
class DailyWaterIntakeScreen extends StatefulWidget {
  ...
  @override
  State<DailyWaterIntakeScreen> createState() => _DailyWaterIntakeScreenState();
}

class _DailyWaterIntakeScreenState extends State<DailyWaterIntakeScreen> {
  double? estimatedIntake; // Untuk menyimpan estimasi kebutuhan air
  ...
  void initState() {
    super.initState();
    _loadUserId(); // Ambil ID pengguna
  }

  Future<void> _loadUserId() async {
    // Ambil ID pengguna dari SharedPreferences
    final prefs = await SharedPreferences.getInstance();
    final id = prefs.getInt('userId'); // Ambil ID pengguna yang disimpan
    if (id != null) {
      setState(() {
        userId = id;
      });
      _fetchEstimatedWaterIntake(id); // Ambil data estimasi kebutuhan air
    } else {
      setState(() {
        isLoading = false;
      });
  ...
  Future<void> _fetchEstimatedWaterIntake(int id) async {
    try {
      final intake = await ApiService.getEstimatedWaterIntake(id);
      setState(() {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Error fetching water intake: $e')),
      );
    }
  }
                      ...
                      children: [
                        ...
                        Container(
                          ...
                          decoration: BoxDecoration(
                            ...
                            color: const Color.fromARGB(255, 248, 242, 245),
                            boxShadow: [
                              BoxShadow(
                        ...
                        // Tampilkan estimasi kebutuhan air
                        Text(
                          estimatedIntake != null
                              ? "${estimatedIntake!.toStringAsFixed(3)} ml"
                              : "0.000 ml",
...
class _HomeScreenState extends State<HomeScreen> {
  Map<String, dynamic>? sensorData; // Data sensor terbaru
  ...
  void initState() {
    super.initState();
    _fetchLastSensorData();
    _fetchUserData();
  }

  // Ambil data sensor
  Future<void> _fetchLastSensorData() async {
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Error fetching sensor data: $e')),
      );
    }
  }
  ...
  Future<void> _fetchUserData() async {
    try {
      const userId = '53'; // Ganti dengan ID pengguna yang valid
      final data = await ApiService.getUserData(userId);
      setState(() {
        userData = data;
      });
    } catch (e) {
                  ...
                  children: [
                      ...
                      child: Center(
                        child: Column(
                          ...
                          children: [
                            ...
                            Text(
                              "/ ${userData?['estimasi_kebutuhan_air'] ?? 2000} L", // Update
                    ...
                    Row(
                      ...
                      children: [
                        _InfoCard(
                          title: "Capacity",
                          value: "${sensorData?['berat_air_sensor'] ?? 0}/1000 mL",
                        ),
                        _InfoCard(
                          title: "Temperature",
                          value: "${sensorData?['suhu_sensor'] ?? 0}°C",
                        ),
                      ],
                    ),
